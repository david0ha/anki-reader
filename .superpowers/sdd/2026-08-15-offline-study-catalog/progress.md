...
- Task 6: runtime review fix round 2/5 — confirmed and closed unchecked FreeRTOS initialization and non-production arbitration coverage; failure injection covers all 13 init stages with zero queue publication on failure.
- Task 6: runtime hardening GREEN — production-used restore/remote/local/source-switch transactions, saturated startup ordering, ASan+UBSan, provisioning, portable 9/9, catalog-store 1/1, simulator link, ESP-IDF objects, and 2 KiB hard frame gate pass; no Task 7-owned or managed-component file modified.
