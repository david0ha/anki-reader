...
{
    const char *body = "ssid=My+Net&password=p%40ss&study_url=http%3A%2F%2Fmac.local%3A8123%2Fvault.json";
    char out[64];
    ...
    CHECK_STR(out, "p@ss");
    CHECK(prov_form_get_field(body, "study_url", out, sizeof(out)) == true);
