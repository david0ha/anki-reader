...
//   2. The mDNS default `http://ankireader.local` (NSBonjourservices _http._tcp / Android mDNS).
...
// The firmware advertises `ankireader`, deliberately NOT the `tickerboard` of the project it
...
export const DEFAULT_HOST = 'ankireader.local'
export const DEFAULT_BASE_URL = `http://${DEFAULT_HOST}`

// host:        a DNS label, a dotted hostname (e.g. ankireader.local), or an IPv4 address.
 ...
 * onboarding and lives on the home LAN, reachable via its DHCP IP or mDNS `ankireader.local`.
 *
 ...
 * (from `/api/info`'s `ip` field) is the preferred candidate and `ankireader.local` is the
