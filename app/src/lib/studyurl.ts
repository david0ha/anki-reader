// Client-side validation of the study card URL, mirroring the firmware's
// `prov_validate_study_url()` (components/provisioning/prov_config.c).
//
...
// write comes back as the single code `study_url_invalid`, which cannot say *what* was wrong.
//
// The firmware's rule, exactly:
//   - empty is valid — it means "run on the built-in demo card"
...
// a bug in one of the two, which is what `studyurl.test.ts` is for.

import { STUDY_URL_MAX_LEN } from './esp32'

export type StudyUrlError = 'too_long' | 'bad_scheme' | 'no_host'

export interface StudyUrlResult {
  ok: boolean
  /** The trimmed URL when ok — this is what should be sent to the board. */
  value?: string
  /** True when the (trimmed) input was empty, i.e. "use the demo card". */
  empty?: boolean
  error?: StudyUrlError
}

/**
 * Validate free text the user typed as a study URL.
 *
 ...
 * looks in the field.
 */
export function validateStudyUrl(input: string): StudyUrlResult {
  const url = (input ?? '').trim()
  if (url.length === 0) return { ok: true, value: '', empty: true }

  if (utf8Length(url) > STUDY_URL_MAX_LEN) return { ok: false, error: 'too_long' }
  ...
  // "http://" and "http:///kanji.json" both parse as URLs and both fail at connect time with an
...
export function studyUrlErrorMessage(result: StudyUrlResult): string {
  switch (result.error) {
    case 'too_long':
      return `That address is too long — the board stores at most ${STUDY_URL_MAX_LEN} characters.`
    case 'bad_scheme':
      return 'The address must start with http:// or https://.'
    case 'no_host':
      return 'The address is missing a host, e.g. http://mymac.local:8123/kanji.json.'
    default:
